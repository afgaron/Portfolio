...
def sample_numbers():
	...
	param_list = [{'RGZ.morphology':'double', 'RGZ.radio_consensus':{'$gte':.65}, 'best.redshift':{'$exists':True}}, \
		{'RGZ.morphology':'double', 'RGZ.radio_consensus':{'$gte':.65}, 'RGZ.size_arcmin':{'$lte':1.5}, 'RGZ.overedge':0, 'using_peaks.bending_angle':{'$lte':135.}, 'best.redshift':{'$exists':True}}, \
		{'RGZ.morphology':'double', 'RGZ.radio_consensus':{'$gte':.65}, 'RGZ.size_arcmin':{'$lte':1.5}, 'RGZ.overedge':0, 'using_peaks.bending_angle':{'$lte':135.}, 'best.redshift':{'$exists':True}}, \
		{'RGZ.morphology':'double', 'RGZ.radio_consensus':{'$gte':.65}, 'RGZ.size_arcmin':{'$lte':1.5}, 'RGZ.overedge':0, 'using_peaks.bending_corrected':{'$lte':135.}, 'best.redshift':{'$exists':True}}, \
		{'RGZ.morphology':'double', 'RGZ.radio_consensus':{'$gte':.65}, 'RGZ.size_arcmin':{'$lte':1.5}, 'RGZ.overedge':0, 'using_peaks.bending_corrected':{'$lte':135.}, 'best.redshift':{'$exists':True}}]
	coll_list = [bent_sources, bent_sources, bending_15, bent_sources, bending_15]
	label_list = ['Consensus cut', 'Size/bending cut', 'Matched', 'Final (all)', 'Final (matched)']
...
def contamination():
	...
	ax1.plot(bins, n, label='Observed')
	ax1.plot(bins, cont_per_bin, ls='--', label='Contamination')
	...
	ax2.set_xlabel('Separation ($r_{500}$)')
...
def orientation_test():
	...
	# model 1: count goes to 0 at 0 deg
	class quad(stats.rv_continuous):
		def _pdf(self, x, a, b, c):
			x0 = 180.
			norm = a*x0**3/3. + b*x0**2/2. + c*x0
			return (a*x**2 + b*x + c) / norm

	a, b, c = 0, n_f[-1]/2., n_f[0]
	...
	class piecewise_lin(stats.rv_continuous):
		def _pdf(self, x, a, b, c):
			...
			m = (c-a) / 90.
			if type(x) is float:
				return (a + 0.5*m*max([x-90., 0])) / norm
			elif type(x) is np.ndarray:
				return (a + 0.5*m*np.max([x-90., np.zeros(len(x))], axis=0)) / norm
			else:
				raise TypeError('Got %s instead'%str(type(x)))

	a, b, c = n_f[-1]/2., n_f[-1]/2., n_f[0]-n_f[-1]/2.
...
def rmsd(params, x_param, y_param, plot=True, coll=bending_15):
	...
	if plot:
		...
		ax2.scatter(run_x, run_rmsd, s=1)
		...
		ax.scatter(run_x, run_rmsd, s=1, label='RMS difference')
		ax.legend(loc='best')
		ax.set_xlabel(get_label(x_param))
...
def asym_comp(align='radial', cut=None):
	...
	mask = np.logical_and(sep>0.01, sep<1.5)
	print 'n:', sum(mask)
	
	popt, pcov = curve_fit(lambda x, a, b: a*x+b, sep[mask], asym[mask])
	perr = np.sqrt(np.diag(pcov))
	print 'slope:', popt[0], '+-', perr[0]

	s = stats.spearmanr(sep[mask], asym[mask])
	print 'r/r500:', s.pvalue, z_score(s.pvalue)

	s = stats.spearmanr(p[mask], asym[mask])
	#s = stats.spearmanr(p[np.logical_and(sep>0.01,p>5e-4)], asym[np.logical_and(sep>0.01,p>5e-4)])
	print 'P:', s.pvalue, z_score(s.pvalue)
	
	s = stats.spearmanr(grad_p[mask], asym[mask])
	...
	for method in ['using_peaks', 'using_contour']:
		...
		for i in coll.find(params):
			...
			if method == 'using_contour':
				bending_15.update({'_id':i['_id']}, {'$set':{method+'.asymmetry':inner/outer, 'WHL.alignment':alignment}})
			else:
...
def sdss_density():
	f, (ax1, ax2) = plt.subplots(1, 2, sharey=True, subplot_kw=dict(adjustable='datalim', aspect='equal'))
	...
	density = n/bin_area
	err = np.sqrt(n)/bin_area
	ax1.errorbar(bins, density, yerr=err, fmt='o', ms=4, label='RGZ')
	logx = np.log10(bins[1:-3])
	logy = np.log10(density[1:-3])
	logyerr = err[1:-3] / density[1:-3]
	fitfunc = lambda p, x: p[0]*x + p[1]
	...
	index_err = np.sqrt(out[1][1][1])
	amp_err = np.sqrt(out[1][0][0])*amp
	ax1.plot(bins, pow(10, index*np.log10(bins)+amp), c='k', label='$x^{%.2f\pm%.2f}$'%(index,index_err))
	ax1.legend(loc='upper right')
	...
	density = n/bin_area
	err = np.sqrt(n)/bin_area
	ax2.errorbar(bins, density, yerr=err, fmt='o', ms=4, label='SDSS')
	logx = np.log10(bins[1:-3])
	logy = np.log10(density[1:-3])
	logyerr = err[1:-3] / density[1:-3]
	out = leastsq(errfunc, [-1,1], args=(logx, logy, logyerr), full_output=1)
	...
	index_err = np.sqrt(out[1][1][1])
	amp_err = np.sqrt(out[1][0][0])*amp
	ax2.plot(bins, pow(10, index*np.log10(bins)+amp), c='k', label='$x^{%.2f\pm%.2f}$'%(index,index_err))
	...
	ax2.set_xscale('log')
	ax.set_xlabel('Separation ($r_{500}$)')
	ax1.set_ylabel('Surface density of galaxies ($r_{500}^{-2}$)')
...
def fractional_bent():
	...
	fig, ax = plt.subplots(1)
	linear_bins = np.insert(pow(10,(bins[:-1]+bins[1:])/2.), 0, [0, np.median(bend[marginal])])
	ratio = n_inner/n_all
	err = ratio * np.sqrt(1/n_inner + 1/n_all)
	plt.errorbar(linear_bins, ratio, yerr=err)
			...
			for objID, ra, dec, z, n_objID, n_ra, n_dec, n_z, z_type, cModelMag_r, fracDeV_r, dist in r:
				...
				if source is not None:
					...
					if np.abs(z-float(n_z))/(1+z) <= 0.04:
						...
						env[objID]['neighbors'][n_objID] = {'ra':float(n_ra), 'dec':float(n_dec), 'z':float(n_z), 'z_type':z_type, 'cModelMag_r':float(plotcModelMag_r), 'fracDeV_r':float(fracDeV_r), 'dist_deg':float(dist), 'dist_Mpc':float(dist)*np.pi/180*ADD}
					if len(env)>count:
						count += 1
						print count
...
def distant_env(bin_count=8, inner_cut=0.05, outer_cut=0.35):
	...
	plt.figure()
	plt.errorbar(bins[1:], frac_bent-1., frac_err_bent, label='Bent sources')
	plt.errorbar(bins[1:]-0.01, frac_straight-1., frac_err_straight, ls='--', label='Straight sources')
...
def to_file(filename):
        with open(filename, 'w') as f:
                print >> f, '''rgz_id,morphology,size,first_id,sdss_ra,sdss_dec,sdss_z,bending_angle,bending_corrected,bending_excess,wh15_separation,wh15_m500,wh15_picm,wh15_orientation,wh15_alignment,asymmetry'''
                for source in bending_15.find(total_cuts):
                        print >> f, source['RGZ']['RGZ_name'], ',', source['RGZ']['morphology'], ',', source['RGZ']['size_arcmin'], ',', source['FIRST']['FIRST_id'], ',', source['best']['ra'], ',', source['best']['dec'], ',', source['best']['redshift'], ',', source['using_peaks']['bending_angle'], ',', source['using_peaks']['bending_corrected'], ',', source['using_peaks']['bending_excess'], ',', source['WHL']['r/r500'], ',', source['WHL']['M500'], ',', source['WHL']['P'], ',', source['WHL']['orientation_peaks'], ',', source['WHL']['alignment'], ',', source['using_contour']['asymmetry']
